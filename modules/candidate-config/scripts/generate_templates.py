...
def add_sender_block_hybrid(doc):
    ...
    run_addr = p_addr.add_run('{{SENDER_STREET}}, {{SENDER_POSTAL_CODE}} {{SENDER_CITY}}')
...
def main():
    parser = argparse.ArgumentParser(description='Generates the neutral Cover Letter template (language-agnostic)')
    parser.add_argument('--style', choices=['hybrid', 'block'], default='hybrid',
                        help='Header style: hybrid (3 lines) or block (6 lines)')
    parser.add_argument('output_dir', nargs='?', default='/mnt/user-data/outputs/',
                        help='Output folder')
    ...
    if args.style == 'hybrid':
        print("  - 3 lignes : nom / adresse complète / coordonnées digitales (email | linkedin | téléphone)")
    else:
        print("  - 6 lignes : nom / rue / code postal+ville / email / linkedin / téléphone")
    
    print(f"\n📋 Placeholders supportés :")
    placeholders = [
        '{{SENDER_NAME}}', '{{SENDER_STREET}}', '{{SENDER_POSTAL_CODE}}',
        '{{SENDER_CITY}}', '{{SENDER_EMAIL}}', '{{SENDER_LINKEDIN}}',
        '{{SENDER_PHONE}}',
        '{{RECRUITER_NAME}}', '{{RECRUITER_TITLE}}', '{{COMPANY_NAME}}',
        '{{DATE_LETTER}}', '{{JOB_TITLE}}',
        '{{SUBJECT_LABEL}}', '{{GREETING}}', '{{CLOSING}}',
        '{{PARAGRAPH_1_INTRO}}', '{{PARAGRAPH_2_CURRENT}}',
        '{{PARAGRAPH_3_EXPERIENCE}}', '{{PARAGRAPH_4_ACHIEVEMENTS}}',
        '{{PARAGRAPH_5_CLOSING}}',
        '{{SIGNATURE_IMAGE}}', '{{SENDER_FULL_NAME}}'
    ]
