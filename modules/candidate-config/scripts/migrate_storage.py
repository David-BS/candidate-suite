...
def main():
    ...
    if action == 'encode_signature':
        if len(args) < 1:
            print("Usage: python migrate_storage.py encode_signature <input_image>", file=sys.stderr)
            sys.exit(1)
        encode_signature(args[0])
    
    elif action == 'copy_to_outputs':
        if len(args) < 1:
            print("Usage: python migrate_storage.py copy_to_outputs <source_path>", file=sys.stderr)
            sys.exit(1)
