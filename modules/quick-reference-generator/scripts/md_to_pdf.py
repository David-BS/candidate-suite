...
def preprocess_tips(md_text):
    ...
    while i < len(lines):
        ...
        if m_box:
            ...
            j = i + 1
            while j < len(lines) and lines[j].startswith('>') and not re.match(r'^>\s*\[!TIP', lines[j]):
...
def preprocess_semantic_bullets(md_text):
    ...
    for line in lines:
        ...
        if m_plus:
            ...
            # Split title — content
            output.append(f'{indent}- <span class="strength-title">{inline_md_to_html(rest)}</span>')
        elif m_minus:
            indent, rest = m_minus.group(1), m_minus.group(2)
            output.append(f'{indent}- <span class="weakness-title">{inline_md_to_html(rest)}</span>')
        else:
            output.append(line)
...
def _columnize_sections(html):
    ...
    def to_table(m):
        ...
        return (prefix + '<table class="cols2"><tr>'
...
def main():
    parser = argparse.ArgumentParser(description='Converts a Markdown file into a styled PDF')
