...
def main():
    parser = argparse.ArgumentParser(description="Generates a quick reference card in Markdown")
    ...
    content_sections = ['pitch_short', 'key_stats', 'top_points',
                        'quick_qa', 'questions_to_ask', 'checklist']
    ...
    if not has_pitch or len(filled) < 4:
        ...
        print(f"   Sections remplies : {filled or 'aucune'} ({len(filled)}/6).", file=sys.stderr)
        print("   La fiche mémo CONDENSE les autres documents : elle doit contenir au minimum", file=sys.stderr)
        print("   le pitch (pitch_short) ET au moins 4 sections parmi : pitch_short, key_stats,", file=sys.stderr)
        print("   top_points, quick_qa, questions_to_ask, checklist.", file=sys.stderr)
        print("   → Reprends les points clés des documents déjà générés (playbook, synthèse,", file=sys.stderr)
    ...
    sections = [k for k in ['pitch_short', 'key_stats', 'top_points',
                            'quick_qa', 'questions_to_ask', 'checklist']
                if data.get(k)]
    ...
    if warnings:
        ...
        for w in warnings:
            print(f"   - {w}")
        print("   → Vérifie le JSON : les clés attendues sont 'point'/'evidence' (top_points)")
