...
def format_answer(answer_text):
    ...
    if has_bullets:
        ...
        md = f'<span class="a-label">{{answer_label}} :</span>'
        if intro:
            md += f' {intro}'
        md += '\n\n'
...
def generate_interview_prep_md(labels, data):
    ...
    for i, qa in enumerate(data.get('screening_questions', []), start=1):
        parts.append(build_question_block(t['question_label'], t['answer_label'], i, qa))
...
def main():
    parser = argparse.ArgumentParser(description="Generates an interview-prep document in Markdown")
    ...
    required = ['candidate_name', 'job_title', 'company_name', 'date',
                'screening_questions', 'competence_questions']
    ...
    print(f"   - {len(data.get('competence_questions', []))} questions de validation compétences")
